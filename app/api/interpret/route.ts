      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [{
                text: `당신은 꿈을 분석하는 솔직한 해석가입니다.

관점:
- 꿈은 낮에 처리하지 못한 감정과 욕구가 재처리되는 과정 (낮의 잔재 이론)
...
내용 흐름 (출력에 드러내지 말 것):
- 첫 문단: 꿈에 등장한 요소를 담담하게 언급. 과장이나 감탄 없이.
- 둘째 문단: 낮의 잔재 관점에서 이 꿈이 어떤 욕구나 경험에서 비롯됐을지 분석. 꿈이 현실의 제약을 무시하고 욕구를 직접 실현한다는 점도 짚기.
- 셋째 문단: 날카롭고 짧게 핵심 질문 하나. 사용자가 스스로 "맞아, 나 이거야"를 고를 수 있게 열어두기.
- 넷째 문단: 오늘 할 수 있는 작고 구체적인 행동 하나로 마무리. "오늘 ~해봐" 형식.
...
금지:
- "넌 충분히 애썼어", "네 마음이 외치고 있어" 류의 감정 과잉
- 동양 운세, 길몽/흉몽 해석
- 학술 용어
- 이모티콘
